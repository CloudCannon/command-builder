...
test('outputs with empty config', () => {
	expect(runScriptCommands()).toEqual([
		...
		'echo "$ cp node_modules/eleventy-plugin-cloudcannon/cloudcannon/inject-cloudcannon.config.js .eleventy.js"',
		'cp node_modules/eleventy-plugin-cloudcannon/cloudcannon/inject-cloudcannon.config.js .eleventy.js',

		'echo "$ export CC_ELEVENTY_INPUT="""',
		'export CC_ELEVENTY_INPUT=""',
...
test('outputs with @next config', () => {
	expect(runScriptCommands({ use_beta_plugin: true })).toEqual([
		'echo "$ node -v"',
		'node -v',
		'echo "$ npm -v"',
		'npm -v',
