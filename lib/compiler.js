module.exports = class Compiler {
	...
	static getOutputCommands(buildOutputPath, preserveSource = false) {
		const outputPath = Compiler.getOutputPath();
		const action = preserveSource ? 'cp -R' : 'mv';

		return [
			`find ${outputPath} -mindepth 1 -delete`,
			'shopt -s dotglob',
			`[ -z "$(ls ${buildOutputPath})" ] || ${action} ${buildOutputPath}/* ${outputPath}`,
			'shopt -u dotglob'
		];
	}
